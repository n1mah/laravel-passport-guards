...
Route::post('/student/token', function () {
    return ['token' => Student::findOrFail(1)?Student::findOrFail(1)->createToken('dev1')->accessToken:null];
});
Route::post('/teacher/token', function () {
    return ['token' => Teacher::findOrFail(1)?Teacher::findOrFail(1)->createToken('dev2')->accessToken:null];
});
Route::post('/admin/token', function () {
    return ['token' => Admin::findOrFail(1)?Admin::findOrFail(1)->createToken('dev3')->accessToken:null];
});
